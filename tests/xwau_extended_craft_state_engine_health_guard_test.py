#!/usr/bin/env python3
"""Reject raw mission/runtime engineEmitterHealth access outside compatibility/preview boundaries."""
...
TOKEN = re.compile(r"->\s*engineEmitterHealth\b")
ALLOWED_FILES = {
...
def main() -> int:
    ...
    if violations:
        print(f"FAIL: {len(violations)} unauthorized raw engineEmitterHealth references")
        for rel, line, excerpt in violations:
            print(f"{rel}:{line}: {excerpt}")
        return 1
    print("PASS: no unauthorized raw engineEmitterHealth references")
    return 0
