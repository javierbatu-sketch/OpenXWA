#!/usr/bin/env python3
"""Reject raw mission/runtime meshRotation access outside the explicit preview/compatibility boundaries."""
...
TOKEN = re.compile(r"->\s*meshRotation\b")
ALLOWED_FILES = {
...
def main() -> int:
    ...
    if violations:
        print(f"FAIL: {len(violations)} unauthorized raw meshRotation references")
        for rel, line, excerpt in violations:
            print(f"{rel}:{line}: {excerpt}")
        return 1
    print("PASS: no unauthorized raw meshRotation references")
    return 0
